...
export function getGeminiApiKey(request: Request): string {
  const fromHeader = request.headers.get(API_HEADER.gemini)?.trim();
  const key = fromHeader || process.env.GEMINI_API_KEY?.trim();
  if (!key) {
    throw new Error(
